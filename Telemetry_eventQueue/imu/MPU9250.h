...
float readTempData()
{
  ...
  temp = (int16_t)(((int16_t)rawData[0]) << 8 | rawData[1]) ;  // Turn the MSB and LSB into a 16-bit value
...
// Configure the accelerometer for self-test
   ...
   wait_us(25); // Delay a while to let the device stabilize
	...
	void MahonyQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz)
{
	...
	q[3] = q4 * norm;
        ...
        pitch *= 180.0f * INVPI;
        yaw   *= 180.0f * INVPI;
        yaw   -= 9.766f; // Declination at Danville, California is 13 degrees 48 minutes and 47 seconds on 2014-04-04
        roll  *= 180.0f * INVPI;
  ...
  MPU9250::MPU9250()
  {
	...
	magCalibration[3] = {0.0f, 0.0f, 0.0f}, magbias[3] = {0.0f, 0.0f, 0.0f};  // Factory mag calibration and mag bias
	gyroBias[3] = {0.0f, 0.0f, 0.0f}, accelBias[3] = {0.0f, 0.0f, 0.0f}; // Bias corrections for gyro and accelerometer

	accelCount[3]= {0, 0, 0};  // Stores the 16-bit signed accelerometer sensor output
	gyroCount[3]= {0, 0, 0};   // Stores the 16-bit signed gyro sensor output
	magCount[3]= {0, 0, 0};    // Stores the 16-bit signed magnetometer sensor output
