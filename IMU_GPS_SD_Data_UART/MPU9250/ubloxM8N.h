...
class ubloxM8N {
    ...
    public:
        ...
        float latitude, longitude, timefix, speed, altitude, cogt,groundSpeedKnts;
        float resF;
        char resC;
        int resI;
    ...
    void parse(char *cmd)
    {
        ...
        // Geographic position, Latitude and Longitude
	   if(strncmp(cmd,"$GNVTG", 6) == 0)
	   {
		   sscanf(cmd, "$GNVTG,%f,%c,%f,%c,%f", &cogt, &resC, &resF, &resC, &speed);
	   }

        // Geographic position, Latitude and Longitude
//	   if(strncmp(cmd,"$GPGSV", 6) == 0)
//	   {
//		   sscanf(cmd, "$GPGSV,%f,%c,%f,%c,%f,%c,%f,,%d", &timefix, &status, &latitude, &ns, &longitude, &ew, &speed, &date);
//	   }
